<template>
  ...
  <dashboard-box v-else :name="chartName" :mBackground="mBackground" @open-widget="handeOpen" @refresh-widget="handeRefresh">
...
let options = {
  ...
  methods: {
    initByWidget(reload) {
      ...
      this.loading = true
      this.$store.dispatch('dashboard/getWidgetData', { widgetData: this.widgetData, filters: this.filters, reload: reload, config: this.widget.widget.data.config }).then(seriesData => {
        this.loading = false
        this.$nextTick(() => {
    ...
    createOption(seriesData) {
      ...
      function parseSeriesData(data, values, keys) {
        ...
        values.forEach(value => {
          ...
          keys.forEach(key => {
            let itemValue = data['F'][value.name][value.aggType][key.join('-')]
