...
<template>
  <div class="content">
    <div class="row">
      ...
      <!-- 目录的添加操作（只有一级目录弹出） -->
      <div class="config-tree-operation" :style="{ top: treeOperationTop, left: treeOperationLeft }" v-show="showTreeAdd">
        <div class="config-tree-operation-item" @click="addLayout('grid')">
          <i class="fa fa-plus toolbar-icon"></i>
          <span>New Grid Layout</span>
        </div>
        <div class="config-tree-operation-item" @click="addLayout('timeline')">
          <i class="fa fa-plus toolbar-icon"></i>
          <span>New Timeline Layout</span>
        </div>
        <div class="config-tree-operation-item" @click="addLayout('gridster')">
          <i class="fa fa-plus toolbar-icon"></i>
      ...
      <!-- 目录的删除操作（只有二级目录弹出） -->
      <div class="config-tree-operation" :style="{ top: treeOperationTop, left: treeOperationLeft }" v-show="showTreeDel">
        <div class="config-tree-operation-item" @click="delLayout('grid')"><i class="fa fa-trash-o toolbar-icon"></i><span>Delete this Layout</span></div>
      </div>

      <!-- 配置面板 -->
      <div class="col-md-9">
        <!-- <router-view /> -->
        <board-config-content v-if="contentVisble" :id="boardId" :categoryId="categoryId"></board-config-content>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    treeData() {
      let treeData = []
      if (!this.categoryList) return treeData
    ...
    delLayout() {
      this.$confirm('是否确认删除？', '警告', {
        ...
        .then(() => {
          ...
          let params = { id: id }
          this.$req.post(this.$api.deleteBoard, params).then(response => {
